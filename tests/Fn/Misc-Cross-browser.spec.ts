...
test.describe('Misc and Cross-browser', () => {
  ...
  test('TC-CB-004 should not log JavaScript console errors on store load @misc @console', async ({ page }) => {
    ...
    const appConsoleErrors = consoleErrors.filter(
      (message) =>
        !message.includes('sentry-staging.timdaythay.com') &&
        !message.includes('Failed to load resource: the server responded with a status of 404') &&
        !(message.includes('Failed to load resource: net::ERR_FAILED') && consoleErrors.some((error) => error.includes('sentry-staging.timdaythay.com')))
    );

    expect(appConsoleErrors, 'Store should not emit JavaScript console errors while loading').toEqual([]);
  });
});
